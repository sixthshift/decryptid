...
export default (state, action) => {
  ...
  if (action.type === 'switch') {
    ...
    return [
      ...hexes,
      ...zip(tileA, tileB).flatMap(([hexA, hexB]) => [
    ...
    return state.map((hex) => {
      if (
        ...
        if (action.type === 'place') {
          entities = sortBy(
            [...hex[entity], { type: action.payload.mode, colour: action.payload.colour }],
            JSON.stringify,
          );
          ...
          entities = sortBy(
            hex[entity].filter(
              (clue) =>
                !(clue.type === action.payload.mode && clue.colour === action.payload.colour),
            ),
            JSON.stringify,
          );
        }
