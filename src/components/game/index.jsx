...
const toDimensions = (model, radius, spacing) => {
  ...
  const board = {
    width:
      ...
      0.75 * hex.width * x + 0.25 * hex.width + spacing * 0.75 * hex.width * (x - 1),
    height:
      // height of the hexes
      // + trailing end of the last hex
      // + spacing between the hexes + 1 due to each column having alternating offsets
      hex.height * y + 0.5 * hex.height + spacing * hex.height * y,
  };
  return { hex, board };
};
...
function Game() {
  ...
  }, [inversion, game, dispatch]);

  return (
    <div className="portrait:h-fit portrait:w-fit portrait:grow landscape:h-full">
      <svg
        width="100%"
        height="100%"
        viewBox={`-${dimensions.hex.width / 2} -${dimensions.hex.height / 2} ${dimensions.board.width} ${dimensions.board.height}`}
      >
        <title>Cryptid game board</title>
